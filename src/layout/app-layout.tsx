...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiCreditCard } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<AccountInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/accounts` : null),
    () => getAccounts({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Account',
      path: isTenantUser ? `/accounts/view/${data?.[0]?.id}` : '/accounts',
      entity: 'account',
